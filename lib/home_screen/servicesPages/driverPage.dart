...
class DriverPage extends StatefulWidget {
  String service;
  int additionalPrice;
  DriverPage({Key? key, required this.service, required this.additionalPrice}) : super(key: key);
...
class _DriverPageState extends State<DriverPage> {
  ...
  List<dynamic> _quantity =[
    ['LTV', 'https://img.icons8.com/color/2x/sedan.png', Colors.red, 0],
    ['LTV with vehicle (+ Rs.300)', 'https://img.icons8.com/color/2x/sedan.png', Colors.orange, 0],
    ['HTV', 'https://img.icons8.com/color/2x/interstate-truck.png', Colors.purple, 0],
  ];
  List<int> _selectedquantity = [];
  late int selectedIndex = 0;
  late String driverType;
  late String service1="0";
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        floatingActionButton: _selectedquantity.length > 0 ? FloatingActionButton(
          onPressed: () {
            if(driverType=="LTV with vehicle (+ Rs.300)")
            {
              service1 = driverType.replaceAll("LTV with vehicle (+ Rs.", "");
              service1 = service1.replaceAll(")", "");
              widget.service = "Driver";
              widget.additionalPrice = int.parse(service1);
              print(widget.additionalPrice);

            }
              Navigator.push(context, MaterialPageRoute(builder: (context) => Frequency(service: widget.service, allServices: driverType, gender: "male", additionalPrice: widget.additionalPrice)),);
